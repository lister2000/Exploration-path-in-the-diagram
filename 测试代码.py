...
class PathDetector:
    ...
    def _build_ui(self):
        ...
        ttk.Label(param_frame, text="二值化算法:").pack(side=tk.LEFT, padx=(10, 2))
        self.algorithm_var = tk.StringVar(value="区域扫描")
        algo_combo = ttk.Combobox(param_frame, textvariable=self.algorithm_var,
                                   values=["双向扫描", "区域扫描"],
        ...
        self.threshold_label = ttk.Label(param_frame, text="100", width=4)
        ...
        self.btn_path = ttk.Button(btn_frame, text="步骤3: 提取路径(高级)", command=self._run_extract_path)
    ...
    def _run_scan(self):
        ...
        self._show_status(f"扫描中... (算法: {algorithm})")
        self.progress_label.config(text=f"扫描中... ({algorithm})")
    ...
    def _scan_thread(self):
        try:
            ...
            if algorithm == "双向扫描":
                binary_img, vertical_binary, horizontal_binary = self._bidirectional_scan(
                    self.img_gray, threshold, min_width, max_width
                )
                scan_type = "双向扫描"
            else:
                binary_img = self._region_scan(
                    self.img_gray, threshold, min_width, max_width
                )
                vertical_binary = None
                horizontal_binary = None
                scan_type = "区域扫描"
            ...
            if algorithm == "双向扫描" and vertical_binary is not None and horizontal_binary is not None:
                ...
                cv2.putText(marked_img, f"算法: {scan_type}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                
                vert_pixels = np.sum(vertical_binary == 255)
                horz_pixels = np.sum(horizontal_binary == 255)
                info_text = f"竖向:{vert_pixels}px | 横向:{horz_pixels}px | 总计:{total_pixels}px"
            else:
                blue_color = np.array([255, 0, 0], dtype=np.uint8)
            ...
            cv2.putText(binary_color, f"算法: {scan_type}", (10, 30), 
            ...
            self.root.after(0, self._show_status, 
                           f"✅ 扫描完成 ({scan_type}): {info_text}")
            self.root.after(0, self._update_progress, 
                           f"{scan_type}: {info_text}")
    ...
    def _bidirectional_scan(self, gray_img, threshold, min_width, max_width):
        h, w = gray_img.shape
        ...
        for x in range(w):
            ...
            for seg in segments:
                ...
                if min_width <= seg_len <= max_width:
                    top_ok = seg[0] == 0 or gray_img[seg[0] - 1, x] > threshold
                    bottom_ok = seg[1] == h - 1 or gray_img[seg[1] + 1, x] > threshold
        ...
        for y in range(h):
            ...
            for seg in segments:
                seg_len = seg[1] - seg[0] + 1
                if min_width <= seg_len <= max_width:
                    left_ok = seg[0] == 0 or gray_img[y, seg[0] - 1] > threshold
                    right_ok = seg[1] == w - 1 or gray_img[y, seg[1] + 1] > threshold
                    if left_ok and right_ok:
                        for x in range(seg[0], seg[1] + 1):
                            horizontal_binary[y, x] = 255
                            binary_img[y, x] = 255
        
        return binary_img, vertical_binary, horizontal_binary
    ...
    def _extract_path_thread(self):
        try:
            ...
            # 使用膨胀后的图（如果有），否则直接使用二值图
            if self.dilated_binary is not None:
                use_img = self.dilated_binary
                use_type = "膨胀后"
            else:
            ...
            self._show_status(f"找到 {path_count} 条路径，使用Dijkstra提取最长路径...")
            ...
            for comp_id in range(1, n_labels):
                ...
                if len(converted_path) >= 10:
                    ...
                    path_infos.append({
                        ...
                        'start': (start_pt[1], start_pt[0]) if start_pt else None,  # (y,x) -> (x,y)
                        'end': (end_pt[1], end_pt[0]) if end_pt else None
                    })
            
            # ===== 5. 去分叉 =====
            self._show_status("去分叉中...")
            ...
            self.root.after(0, self._show_status, f"✅ 路径提取完成 (高级算法): {path_info}")
    ...
    def _draw_paths_on_image_advanced(self, img, paths, colors):
        ...
        for idx, path in enumerate(paths):
            ...
            for i in range(len(valid_path) - 1):
                cv2.line(img, valid_path[i], valid_path[i+1], color, 2)
            
            # 起点：黄色空心圆
            cv2.circle(img, valid_path[0], 6, (0, 255, 255), 2)
            
            # 终点：黄色实心圆
            cv2.circle(img, valid_path[-1], 6, (0, 255, 255), -1)
