...
MAX_BRANCH_LENGTH = 6 #毛刺的阈值
MIN_BRANCH_KEEP_LENGTH = 0  # 叉路长度阈值: 小于该值的叉路会被忽略
CORE_CLOSED_RATIO = 0.6  # 主干核心占比超过该阈值时，单一路径判为封闭主路
...
def choose_closed_anchor(mask, main_path, junctions):
    ...
    # Prefer a regular loop point (degree==2) that is not on any detected junction.
    loop_points = []
...
def branch_junctions_from_main(mask, main_path, min_branch_keep_length=0):
    ...
    # Merge very close junctions to avoid duplicate circles around one fork.
    merged = []
...
def main():
    ...
    # Step 2: 每条独立骨骼连通域算一条路径，并统计该路径叉路
    ...
    print(f"叉路长度阈值(保留): {MIN_BRANCH_KEEP_LENGTH}")
    print(f"整体判定: {'封闭路径' if image_is_closed else '开放/非封闭路径'}")
    for info in path_infos:
        path_type = '封闭' if info['is_closed'] else '开放'
        print(
            f"路径{info['id']}: 类型={path_type}, 像素={info['pixels']}, 叉路数={info['branch_count']}, "
            f"分叉口数={info['junction_count']}, 最长叉路={info['longest_branch_length']}, "
            f"最短叉路={info['shortest_branch_length']}, 主路颜色={info['main_color']}, 叉路颜色={info['branch_color']}, 起点={info['start']}, 终点={info['end']}"
        )
    print(f"结果已保存: {OUTPUT_PATH}")

    display_img, scale = fit_to_screen(result)
    if scale < 1.0:
        print(f"显示缩放比例: {scale:.3f}")
